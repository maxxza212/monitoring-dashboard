<template>
        ...
        <template #item.kelembapan_batasan="{ item }">
          <div v-if="item.batasan" class="text-caption">
            <v-chip size="small" color="teal" variant="tonal">
              {{ item.batasan.kelembapan_min }}% -
              {{ item.batasan.kelembapan_max }}%
            </v-chip>
          </div>
          <div v-else class="text-grey text-caption">
            <v-icon
  ...
  <v-dialog v-model="dialog" max-width="700" persistent>
    <v-card>
      <v-card-title
        class="text-white"
        style="background: linear-gradient(135deg, #34e89e 0%, #0f3443 100%)"
      >
      ...
      <v-card-text class="pt-4">
        <div class="mb-4">
          ...
          <v-select
            v-model="formData.id_alat"
            :items="devicesList"
            item-title="nama_alat"
            item-value="id"
            label="Pilih Alat"
            prepend-inner-icon="mdi-devices"
            variant="outlined"
            :rules="[(v) => !!v || 'Alat wajib dipilih']"
            required
          ...
          <v-text-field
            v-model="formData.nama_sensor"
            label="Nama Sensor"
            placeholder="Contoh: DHT22 Sensor 1"
            prepend-inner-icon="mdi-memory"
            variant="outlined"
            :rules="[(v) => !!v || 'Nama sensor wajib diisi']"
            required
          ></v-text-field>
        </div>
        ...
        <div>
          ...
          <v-alert type="info" variant="tonal" density="compact" class="mb-3">
            Atur batasan nilai normal untuk sensor ini. Jika tidak diisi, sistem
            akan menggunakan batasan default.
          </v-alert>
          ...
          <v-row>
            <v-col cols="12" md="6">
              ...
              <v-row dense>
                <v-col cols="6">
                  <v-text-field
                    v-model.number="formData.suhu_min"
                    label="Min"
                    type="number"
                    step="0.1"
                    density="compact"
                ...
                <v-col cols="6">
                  <v-text-field
                    v-model.number="formData.suhu_max"
                    label="Max"
                    type="number"
                    step="0.1"
                    density="compact"
            ...
            <v-col cols="12" md="6">
              ...
              <v-row dense>
                <v-col cols="6">
                  <v-text-field
                    v-model.number="formData.kelembapan_min"
                    label="Min"
                    type="number"
                    step="0.1"
                    density="compact"
                ...
                <v-col cols="6">
                  <v-text-field
                    v-model.number="formData.kelembapan_max"
                    label="Max"
                    type="number"
                    step="0.1"
                    density="compact"
  ...
  <v-dialog v-model="dialogDelete" max-width="400">
    <v-card>
      ...
      <v-card-text>
        ...
        <strong>{{ itemToDelete?.nama_sensor }}</strong
        >?
...
const { loadAllBatasan, getBatasanBySensorId, createBatasan, updateBatasan } =
  useBatasan();
...
const filteredSensorList = computed(() => {
  ...
  return sensorList.value.filter(
    (sensor) => Number(sensor.id_alat) === numeric
  );
});
  ...
  try {
    ...
    if (response.data.success) {
      sensorList.value = response.data.data.map((sensor) => {
        ...
        const namaAlat =
          devicesMap.value.get(sensor.id_alat) || `Alat ${sensor.id_alat}`;
...
const submitForm = async () => {
  ...
  try {
    ...
    const hasBatasanData =
      formData.value.suhu_min !== null &&
